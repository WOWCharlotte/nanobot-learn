...
当上下文接近模型上下文窗口上限/设定的阈值时，异步触发压缩，使用LLM做一次摘要总结，再用这个摘要重启上下文窗口。压缩策略的关键在于**保留什么？丢弃什么？** 保留**用户指令、任务状态、中间的重大发现、错误信息**，丢弃**已经完成的工具调用和原始返回结果**
...
### 上下文隔离
当一个任务极度复杂时，即便做了压缩好文件系统，单个上下文窗口也可能不够。这种情况下，可以借助多Agent架构，主Agent负责任务规划，每个子Agent负责执行具体任务并维护独立的干净的上下文窗口。

### 仅追加提高 KV Cache 命中率
确保整体上下文结构顺序不变，系统指令->工具定义->每轮会话信息，这样的好处在于可以提高 KV Cache 命中率，**节省成本**。
